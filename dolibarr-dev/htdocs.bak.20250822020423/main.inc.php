...
// Early include: start server-side branding filter as soon as possible so
// it can capture and rewrite any HTML output (login page strings, titles, anchors).
// Use DOL_DOCUMENT_ROOT path as filefunc.inc.php defines environment constants.
if (isset($DOL_DOCUMENT_ROOT) || defined('DOL_DOCUMENT_ROOT')) {
	$brandFile = DOL_DOCUMENT_ROOT . '/theme/digitalpin_server_branding.php';
	if (is_readable($brandFile)) {
		// include inside a try-safe block; the branding file itself starts output buffering
		include $brandFile;
	}
}

// Fallback: try to include branding file relative to htdocs directory if DOL_DOCUMENT_ROOT wasn't usable yet
$brandCandidates = array();
if (defined('DOL_DOCUMENT_ROOT')) {
	$brandCandidates[] = DOL_DOCUMENT_ROOT . '/theme/digitalpin_server_branding.php';
} elseif (isset($DOL_DOCUMENT_ROOT)) {
	$brandCandidates[] = $DOL_DOCUMENT_ROOT . '/theme/digitalpin_server_branding.php';
}
$brandCandidates[] = __DIR__ . '/theme/digitalpin_server_branding.php';
foreach ($brandCandidates as $brandFileCandidate) {
	if (!empty($brandFileCandidate) && is_readable($brandFileCandidate)) {
		@include $brandFileCandidate;
		break;
	}
}
...
// Inject Digital PIN color system into HTML head for all pages
// This ensures the CSS variables are available even before theme CSS is loaded.
if (isset($conf)) {
	$dp_css = "<style> :root {" .
		"--dp-primary: #2ecc71; --dp-bright: #3498db; --dp-bg: #f9f9f9; --dp-dark: #1a2a3a;" .
		"--dp-slate: #7f8c8d; --dp-accent: #f1c40f; --dp-success: #27ae60; --dp-warning: #f39c12; --dp-error: #e74c3c; }" .
		"</style>";
	if (empty($conf->global->MAIN_HTML_HEADER) || strpos($conf->global->MAIN_HTML_HEADER, '<!--DP_COLOR_INJECTED-->') === false) {
		$conf->global->MAIN_HTML_HEADER = (empty($conf->global->MAIN_HTML_HEADER) ? '' : $conf->global->MAIN_HTML_HEADER) . "<!--DP_COLOR_INJECTED-->" . $dp_css;
	}
}

// Server-side branding filter is included earlier to ensure output buffering
// captures all HTML (see early include above). Removed duplicate late include.
